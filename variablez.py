...
VARIABLES
	...
	=> in python there are number of operations are invoved when working with variables
		1) 	Variable declaration and assignment
		2)  Variable multiple assignment
		3)	Variable deletion via del statement
	=> As variables used to store data to be used in a program, they introduce concept of data types as follows
		a)	Numbers
		b)	String
		c)	List
		d)	Tuple
...
try:
	del number_one
	print('\tnumber_one: {}'.format(number_one)) #This line will throw NameError Exception
except NameError:
...
print("\tIs studentOne string not a member of studentTwo string? => " + str(studentOne not in studentTwo))
print("\tIs studentOne string a member of studentThree string? => " + str(studentOne in studentThree))
print("\tIs a studentOne equal to studentThree? => " + str(studentOne == studentThree))
...
temp_string = str(number) #This converts a number into an equivalent string and store the result in a "temp_string" variable

for i in range(len(temp_string)):
	digits.append(int(temp_string[i])) #This line access each digit in a string, convert it into an integer and lastly insert into a list
